...
awk -F' ' 'NR==FNR{a[$1];next} $1 in a' pmid_list.txt bioconcepts2pubtatorcentral_cleaned > bioconcepts2pubtatorcentral_pmid_cleaned

echo "Regroupement des MESH par PMID"
awk -F' ' '$1==last {printf " %s",$2; next} NR>1 {print "";} {last=$1; printf "%s",$0;} END{print "";}' bioconcepts2pubtatorcentral_pmid_cleaned > bioconcepts2pubtatorcentral_cleaned_grouped

echo "Récupération des noms de MESH"
awk -F' ' '{print $2}' bioconcepts2pubtatorcentral_pmid_cleaned | sort -u > mesh

echo "Nombre de MESH différents : " `wc -l mesh`

echo "Nombre complet d'instances de MESH: " `wc -l bioconcepts2pubtatorcentral_pmid_cleaned`
...
input_file="bioconcepts2pubtatorcentral_cleaned_ms=0.1_mc=0.7.mma.nom"
