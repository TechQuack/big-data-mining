...
tail -n +2 termList.txt | head -n 150 | awk -F'\t' '{gsub(/^[ \t]+|[ \t]+$/, "", $3); print $3}' | sort | uniq > frequent_terms.txt
sed -i 's/^INFunknownSUP//g' frequent_terms.txt
sed -i 's/INFunknownSUP$//g' frequent_terms.txt
sed -i 's/[[:space:]]//g' frequent_terms.txt

# Supprimer les termes égaux à INFunknown et les lignes vides
sed -i '/^INFunknown$/d' frequent_terms.txt
sed -i '/^$/d' frequent_terms.txt  
...
# Lire les termes fréquents dans un tableau et supprimer les doublons
mapfile -t terms <  frequent_terms.txt
...
echo -n "PMID" > frequent_terms.csv
for term in "${clean_terms[@]}"
do
    echo -n ",$term" >> frequent_terms.csv
done
echo "" >> frequent_terms.csv
...
# On redirige le résultat dans le fichier frequent_terms.csv
while read line
do
    ...
    do
        ...
        then
            echo -n ",1" >> frequent_terms.csv
        else
            echo -n ",?" >> frequent_terms.csv
        fi
    done
    echo "" >> frequent_terms.csv
done < result.txt